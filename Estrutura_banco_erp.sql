/* 
   SISTEMA PDV & MINI-ERP - VERS O 3.3 (GOLD REVISION FINAL -)
   = */

USE master;
GO

-- 1. RESET DO BANCO (Cuidado: Apaga tudo para subir a vers o limpa)
...
-- Configura  o de seguran a para garantir integridade em caso de erro
SET XACT_ABORT ON; 
GO
...
--  ndice para busca instant nea por c digo de barras (essencial para PDV)
...
CREATE TABLE Vendas (
    ...
    valor_desconto DECIMAL(10,2) NOT NULL DEFAULT 0 -- Permite descontos no balc o
);
...
CREATE TABLE PagamentosVenda (
    ...
    forma_pagamento VARCHAR(50) NOT NULL, -- DINHEIRO, PIX, CART O
...
--  ndice de cobertura para relat rios r pidos de vendas
...
CREATE TABLE Historico_Movimentacao_Estoque (
    ...
    tipo_movimento VARCHAR(10) NOT NULL, -- ENTRADA / SA DA
    quantidade INT NOT NULL,
...
-- =========================================================================
-- 3. PROCEDURES (L GICA E AUTOMA  O)
-- =========================================================================

-- 3.1 SALVAR PRODUTO (Cria  o Din mica de Marca/Categoria)
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Gerador de c digo interno se n o fornecido
...
BEGIN
    ...
    IF @id_p IS NULL RAISERROR('Produto n o encontrado.', 16, 1);
    ...
    BEGIN TRY
        ...
        -- Registro na Auditoria de Sa da
        INSERT INTO Historico_Movimentacao_Estoque (id_produto, tipo_movimento, quantidade, motivo)
        VALUES (@id_p, 'SA DA', @quantidade, 'Venda ID: ' + CAST(@id_venda AS VARCHAR));

        -- Atualiza o valor total bruto da venda no cabe alho
...
-- =========================================================================
-- 4. VIEWS (RELAT RIOS E DASHBOARDS)
-- =========================================================================

-- 4.1 Cat logo Geral de Produtos
...
-- 4.3 Fechamento de Caixa (Confer ncia de Gaveta)
...
-- 4.4 DRE Mensal (Lucro L quido Real - VIS O ENXUTA SEM MOSTRAR DESCONTO)
CREATE OR ALTER VIEW vw_RelatorioMensal_DRE AS
SELECT 
    Vendas_Agrupadas.Mes,
    
    -- O Faturamento j  aparece com o desconto abatido (Escondendo a coluna de desconto)
    ...
    -- O Lucro L quido j  considera a subtra  o do desconto
